...
@section('contentContainer')
 ...
 <div class="portlet">
		...
		@endforeach
          ...
          {{ Form::open(array('class'=>'form-horizontal','role'=>'form','method'=>'put','route'=>array('admin.lesson.update',$veriler['en']->id))) }}
        ...
        <div class="portlet-content">
		     ...
		     <div class="form-group">
              ...
              <div class="col-md-10">
				...
				@if ($veriler[$lang->prefix]->top_lesson==null)	
					<option value="" selected>Üst Ders Yok</option>
				@else
					<option value="">Üst Ders Yok</option>
				@endif
				@foreach($topLessons as $topLesson)
				@if ($lang->prefix==$topLesson->lang)
					@if ($veriler[$lang->prefix]->top_lesson==$topLesson->id)	
						<option value="{{ $topLesson->id }}" selected>{{ $topLesson->name }}</option>
					@else
						<option value="{{ $topLesson->id }}">{{ $topLesson->name }}</option>
					@endif
					@endif
				@endforeach
              </select>
			...
			<div class="form-group">
              ...
              <div class="col-md-10">
                <select id="s2_basic" class="form-control s2_basic" name="categories_id[{{ $lang->prefix }}]">                    
					...
					@else
						<option value="">Kategorili Ders İçerikleri</option>
					@endif
			...
			<div class="form-group">
              ...
              <div class="col-md-10">
                <select id="s2_basic" class="form-control s2_basic" name="content_id[{{ $lang->prefix }}]">                    
					@if ($veriler[$lang->prefix]->content_id=="0")
						<option value="" selected>Ders İçeriği</option>
					@else
						<option value="">Ders İçeriği</option>
					@endif
				@foreach($contents as $content)
				@if ($lang->prefix==$content->lang)
					@if ($veriler[$lang->prefix]->content_id==$content->id)
			...
			<div class="form-group">
              ...
              <div class="col-md-10">
                <input type="text" class="form-control" name="date[{{ $lang->prefix }}]" value="{{ $veriler[$lang->prefix]->date }}" />
              </div>
            </div>
